...
func WithIdempotency(k string) InvokeOption {
	return func(r *functionv1.InvokeRequest) { r.IdempotencyKey = k }
}

// WithRoute sets route semantics: lb|broadcast|targeted|hash
func WithRoute(v string) InvokeOption {
	return func(r *functionv1.InvokeRequest) { meta(r)["route"] = v }
}

// WithTargetService sets targeted routing service id.
func WithTargetService(id string) InvokeOption {
	return func(r *functionv1.InvokeRequest) { meta(r)["target_service_id"] = id }
}

// WithHashKey sets routing hash key.
func WithHashKey(k string) InvokeOption {
	return func(r *functionv1.InvokeRequest) { meta(r)["hash_key"] = k }
}

// WithTrace sets trace id.
func WithTrace(t string) InvokeOption {
	return func(r *functionv1.InvokeRequest) { meta(r)["trace_id"] = t }
}
...
func (i *Invoker) Invoke(ctx context.Context, functionID string, payload []byte, opts ...InvokeOption) ([]byte, error) {
	...
	// defaults
	if i.cfg.GameID != "" {
		meta(req)["game_id"] = i.cfg.GameID
	}
	if i.cfg.Env != "" {
		meta(req)["env"] = i.cfg.Env
	}
	// apply options
	for _, o := range opts {
		o(req)
	...
	if sch := i.schemas[functionID]; sch != nil {
		...
		if err := ValidateJSON(sch, val); err != nil {
			return nil, err
		}
	}
...
func (i *Invoker) SetSchema(functionID string, schema map[string]any) {
	if i.schemas == nil {
		i.schemas = map[string]map[string]any{}
	}
...
func (i *Invoker) attachHeaders(ctx context.Context, extra map[string]string) context.Context {
	...
	if i.cfg.AuthToken != "" {
		headers["authorization"] = "Bearer " + i.cfg.AuthToken
	}
	if i.cfg.HeaderFunc != nil {
		if m := i.cfg.HeaderFunc(ctx); m != nil {
			for k, v := range m {
				headers[strings.ToLower(k)] = v
			}
		}
